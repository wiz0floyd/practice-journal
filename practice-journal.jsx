...
// ── Default content (replace via the Repertoire editor) ───────────────────────
...
const isDue      = (c) => (c.sessionsUntilDue ?? 0) === 0;
const formatDue  = (n) => !n || n <= 0 ? "due now" : n === 1 ? "next session" : `in ${n} sessions`;
const longDate   = () => new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
const emptyDraft = () => ({ composer: "", title: "", detail: "" });
const draftValid = (d) => d.composer.trim() && d.title.trim();
...
export default function App() {
  ...
  useEffect(() => { save(KEYS.items,   items);   setCards((prev) => syncCards(items, prev)); }, [items]);
  ...
  const addItem = () => {
    ...
    setItems((prev) => [...prev, { id: newId(), composer: newDraft.composer.trim(), title: newDraft.title.trim(), detail: newDraft.detail.trim() }]);
